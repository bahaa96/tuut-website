  ...
  const getFilteredAndSortedStores = useCallback(() => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'deals':
          const aDeals = a.active_deals_count || a.deals_count || a.total_offers || 0;
          const bDeals = b.active_deals_count || b.deals_count || b.total_offers || 0;
    ...
    return (
      <Link href={`/store/${slug}`}>
        <div className={`group bg-white rounded-2xl border-2 border-[#111827] shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] hover:shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] transition-all overflow-hidden`}>
          <div className={`flex ${isRTL ? 'flex-row-reverse' : ''} items-center gap-6 p-6`}>
            {/* Logo */}
