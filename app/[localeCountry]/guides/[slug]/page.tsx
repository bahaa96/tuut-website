...
export async function generateMetadata({
  ...
  if (!description && article.content_markdown) {
    // Extract first 160 characters from content
    const textContent = article.content_markdown.replace(/<[^>]*>/g, ""); // Remove HTML tags
    description =
...
function generateHreflangTagsForGuide(
  slug: string,
  localeCountry: string
  ...
  // Default language
  hreflangs["x-default"] = `https://tuut.shop/en-${
...
export default async function GuidePage({ params }: GuidePageProps) {
  ...
  const faqs = [
    {
      question: isArabic
        ? "ما هي أفضل الطرق لتوفير المال أثناء التسوق؟"
        : "What are the best ways to save money while shopping?",
      answer: isArabic
    ...
    {
      question: isArabic
        ? "كيف أجد أفضل العروض في منطقتي؟"
        : "How do I find the best deals in my area?",
      answer: isArabic
      ...
      <div className="min-h-screen bg-background">
        <div className="container mx-auto max-w-4xl px-4 md:px-6 lg:px-8 py-8 md:py-12">
          ...
          <div className={`mb-8 ${isRTL ? "text-right" : "text-left"}`}>
            ...
            {/* Author Meta */}
            <div
              className={`flex flex-wrap items-center gap-6 text-sm text-[#6B7280] ${
                isRTL ? "flex-row-reverse" : ""
              }`}
            >
              <div
              ...
              >
                ...
                <span>
                  {readingTime} {language === "en" ? "min read" : "دقيقة قراءة"}
                </span>
              </div>
            </div>
          </div>
                  ...
                  >
                    <div className="group bg-white rounded-2xl border-2 border-[#111827] shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] hover:shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] transition-all overflow-hidden h-full flex flex-col">
                      ...
                      >
                        {/* Article Title */}
                        <h3
                          className="mb-3 text-[#111827] group-hover:text-[#5FB57A] transition-colors line-clamp-2"
                        ...
                        {/* Article Meta */}
                        <div
                          className={`flex flex-wrap items-center gap-3 text-xs text-[#6B7280] pt-4 border-t border-[#E5E7EB] ${
                            isRTL ? "flex-row-reverse" : ""
                          }`}
                        >
          ...
          <div className="mt-12">
            <Link href="/guides">
              <Button className="bg-[#5FB57A] hover:bg-[#4CAF50] text-white border-2 border-[#111827] shadow-[4px_4px_0px_0px_rgba(17,24,39,1)] hover:shadow-[2px_2px_0px_0px_rgba(17,24,39,1)] transition-all">
                <BookOpen className="h-4 w-4 mr-2" />
                {language === "en"
                  ? "More Shopping Guides"
                  : "المزيد من أدلة التسوق"}
              </Button>
            </Link>
          </div>
        </div>
