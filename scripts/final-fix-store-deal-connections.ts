...
// Final fix: Create deals for country-specific store variants
import { createClient } from '../utils/supabase/client';

async function finalFixStoreDealConnections() {
  console.log('🔧 Final Fix: Creating Deals for Country-Specific Stores...\n');
  ...
  try {
    ...
    // 4. Create deals for country-specific stores
          ...
          for (const countryStore of countryStores) {
            ...
            if (!dealsByStoreId.has(countryStore.id)) {
              ...
              const dealsToCreate = mainStoreDeals.slice(0, 5).map((deal, index) => {
                const discountPercentage = deal.discount_percentage || (15 + Math.random() * 20);
                const originalPrice = deal.original_price || (100 + Math.random() * 400);
                ...
                return {
                  ...
                  discounted_price: Math.round(originalPrice * (1 - discountPercentage / 100)),
              ...
              if (insertError) {
                ...
                console.log(`   ✅ Created ${dealsToCreate.length} deals for ${countryStore.title}`);
                totalDealsCreated += dealsToCreate.length;
                totalStoresUpdated++;
              }
            } else {
