...
export async function createDeal(
  deal: Omit<Deal, "id" | "created_at" | "user_id">
): Promise<{ data: Deal | null; error: Error | null }> {
  try {
    const supabase = createClient();
...
export async function fetchDealsByCountrySlug(
  countrySlug: string,
  options?: {
    category?: string;
    store?: string;
  ...
  try {
    console.log(
      "🐛 DEBUG fetchDealsByCountrySlug - Received countrySlug:",
      countrySlug
    );
    const supabase = createClient();
    ...
    console.log(
      "🐛 DEBUG fetchDealsByCountrySlug - Query built with country_slug filter:",
      countrySlug
    );
    ...
    console.log(
      "🐛 DEBUG fetchDealsByCountrySlug - Raw data count:",
      data?.length || 0
    );
    console.log(
      "🐛 DEBUG fetchDealsByCountrySlug - Sample raw country_slugs:",
      data?.slice(0, 3).map((d: any) => d.country_slug)
    );
...
export async function fetchFooterFeaturedDeals(
  countrySlug?: string
      ...
      data
        ?.map((item: any) => {
          ...
          return {
            ...
            title_en: deal.title_en,
            discount_percentage:
              deal.discount_unit === "percentage" ? deal.discount : null,
            code: deal.code,
          };
        })
        .filter(Boolean) || [];
...
export async function fetchFooterTopStores(
  countrySlug?: string
): Promise<{ data: any[]; error: Error | null }> {
  try {
    const supabase = createClient();
...
export async function fetchFooterArticles(countrySlug?: string): Promise<{
  data: { id: string; title_en: string; slug: string }[];
  error: Error | null;
}> {
  try {
    const supabase = createClient();
...
export async function fetchArticles(
  ...
  try {
    ...
    if (searchQuery) {
      query = query.or(
        `title.ilike.%${searchQuery}%,excerpt.ilike.%${searchQuery}%,content_markdown.ilike.%${searchQuery}%`
      );
    }
...
export async function fetchFooterBestSellingProducts(
  countrySlug?: string
  ...
  try {
    ...
    const dealWithStore = data
      ? {
          ...
          discount_percentage:
            data.discount_unit === "percentage" ? data.discount : null,
          discount_amount:
            data.discount_unit === "amount" ? data.discount : null,
          code: data.code,
...
export async function fetchFooterData(countrySlug?: string) {
  const [featuredDeals, topStores, articles, categories, bestSellingProducts] =
    await Promise.all([
...
export async function fetchStoresByCountrySlug(
  countrySlug: string,
  options?: {
    limit?: number;
    offset?: number;
...
export async function loadDealsAndDisplay() {
  ...
  const { data: searchResults, error: searchError } = await searchDeals(
    "iPhone",
    { limit: 5 }
  );
