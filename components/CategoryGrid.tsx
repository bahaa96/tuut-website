...
import { 
  ...
  Smartphone, Tablet, Headphones, Monitor, Camera, Gamepad2,
  Utensils, Pizza, Wine, IceCream2, ChefHat,
  Palmtree, Hotel, Luggage, Map, Compass, Globe,
  Sparkle, Brush, Flower2, Gem, Wand2,
  Trophy, Bike, Activity, Target, Waves, CircleDot,
  Sofa, Lamp, Armchair, Frame, Drill, PaintBucket,
  Stethoscope, Pill, HeartPulse, Baby, Leaf, Apple,
  BookOpen, GraduationCap, Pencil, Palette, Music, Film,
  Car, Wrench, Fuel, ShoppingCart, Gift, Tags,
  Cookie, Cat, Dog, PawPrint, Bird, Package,
  Trees, Flower, Shovel, Scissors, Hammer, Zap, LucideIcon
} from "lucide-react";
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="container mx-auto max-w-[1200px] px-4 md:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-white rounded-xl p-6 border-2 border-[#E5E7EB] h-32 animate-pulse" />
            ))}
          </div>
        </div>
      </section>
            ...
            <div className="grid grid-rows-2 grid-flow-col gap-4 md:gap-6 pb-2">
              {categories.map((category, index) => {
                ...
                return (
                  <button
                    key={category.id}
                    onClick={() => router.isReady && router.push(categoryUrl)}
                  ...
                  >
                    ...
                    <div style={{ fontSize: '18px', fontWeight: 600 }} className="text-[#111827] mb-1">
                      {categoryName}
                    </div>
