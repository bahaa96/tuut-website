    ...
    <section className="py-12 md:py-16 bg-white">
      <div className="container mx-auto max-w-[1200px] px-4 md:px-6 lg:px-8">
        ...
        <div className={`text-center mb-10 ${isRTL ? 'text-right' : 'text-left'}`}>
          <h2 className="mb-4 text-[#111827]" style={{ fontSize: '36px', fontWeight: 700 }}>
            {language === 'en' ? (
              <>
                How we're <span className="italic text-[#5FB57A]">different</span> from traditional coupon sites
              </>
            ) : (
              <>
                كيف نحن <span className="italic text-[#5FB57A]">مختلفون</span> عن مواقع القسائم التقليدية
              </>
            )}
          </h2>
        </div>
          ...
          {features.map((feature, index) => {
            ...
            return (
              ...
              >
                ...
                <div className={`absolute ${index === 0 ? '-bottom-6 -left-6' : index === 1 ? 'bottom-4 right-4' : 'top-1/2 left-4'} w-16 h-16 rounded-full bg-white/10 border-2 border-white/30`} />
                
                <div className={`flex flex-col ${isRTL ? 'items-end' : 'items-start'} h-full relative z-10`}>
                  <div 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <Card className="p-8 border-2 border-[#111827] rounded-xl shadow-[3px_3px_0px_0px_rgba(17,24,39,1)] bg-gradient-to-br from-[#BCF0CC] to-[#7EC89A] overflow-hidden relative">
            <div className={`flex flex-col ${isRTL ? 'items-end' : 'items-start'}`}>
              <div className="flex items-baseline gap-2 mb-4">
                <span className="text-[#111827]" style={{ fontSize: '64px', fontWeight: 700 }}>
                  4.7
                </span>
              ...
              <p className={`text-sm text-[#6B7280] ${isRTL ? 'text-right' : 'text-left'}`}>
                {language === 'en' ? 'Last updated Aug 30, 2025' : 'آخر تحديث 30 أغسطس 2025'}
              </p>
            </div>
            {/* Decorative circles */}
          ...
          <Card className="p-8 border-2 border-[#111827] rounded-xl shadow-[3px_3px_0px_0px_rgba(17,24,39,1)] bg-gradient-to-br from-[#5FB57A] to-[#4FA569] overflow-hidden relative">
            <div className={`flex ${isRTL ? 'flex-row-reverse' : 'flex-row'} items-center gap-6 h-full`}>
              <div className="flex-1">
                ...
                <p className={`text-white/90 mb-4 text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                  {language === 'en'
                    ? 'Be the hero other shoppers need.'
                    : 'كن البطل الذي يحتاجه المتسوقون الآخرون.'}
                </p>
                <p className={`text-white/80 mb-6 text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                  {language === 'en'
